...
define(function (require) {
    ...
    prototype.button = function () {
        var self = this;
        var callback = self.__callback;
        var args = [].slice.call(arguments);

        $.each(args, function (index, val) {
            var $a;
            var id;
            var className = 'ui-dialog-button';
            var options;
            var isNew;
    ...
    prototype.__click = function (id) {
        var self = this;
        var fn = self.__callback[id] && self.__callback[id].callback;
    ...
    prototype.position = function () {
        ...
        var dl = fixed ? 0 : $window.scrollLeft();
        var dt = fixed ? 0 : $window.scrollTop();
        var ww = $window.width();
        ...
        wrap.css({
            left: Math.max(parseInt(left, 10), dl),
            top: Math.max(parseInt(top, 10), dt)
        });
